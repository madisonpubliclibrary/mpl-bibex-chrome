  ...
  if (/cgi-bin\/koha\/members\/memberentry\.pl/.test(window.location)) {
    ...
    class PSTATs {
      constructor() {
        this.data = {
          ...
          "Green": {
            ...
            "Brodhead city": "G-BROD-C",
            "Browntown village": "G-MRO-SD",
            "Cadiz town": "G-CAD-T",
            "Clarno town": "G-MRO-SD",
            "Decatur town": "G-DEC-T",
            "Exeter town": "G-EXE-T",
            "Jefferson town": "G-JEF-T",
            "Jordan town": "G-JOR-T",
            "Monroe city": "G-MRO-SD",
            "Monroe town": "G-MRO-SD",
      ...
      /**
         ...
         */
      find(county, countySub) { this.find(county, countySub, ""); };
      ...
      /**
         ...
         */
        find(county, countySub, censusTract) {
    ...
    /**
     * The SCLSLibs object contains the addresses of all SCLS locations, grouped
     * by MPL and County. It also provides a function to query the Google Maps API.
     *
     * @constructor
     * @author Lucas Schneider
     * @this {SCLSLibs}
     * @return {SCLSLibs} The new SCLSLibs object
     */
    class SCLSLibs {
      constructor() {
        this.data = {
    ...
    class PSTATmsg {
      constructor() {
        // Message colors
        this._searching = "#337AB7";
        this._success = "#00c000";
        this._error = "#c00c00";
      }
    ...
    // Instantiate classes
    const pstats = new PSTATs();
    const sclsLibs = new SCLSLibs();
    const pstatMsg = new PSTATmsg();

    // Define variable for target address (primate or alternate)
    let targetAddr;
    let targetCity;
    let targetZip;
    ...
    // Variables for selecting nearest "home" library
    ...
    nearestLib.onclick = function() {
      const sclsLibs = new SCLSLibs();
      const selected = document.getElementById('mapRegionList').selectedOptions[0].value;

      chrome.runtime.sendMessage({
        "key": "findNearestLib",
        "address": encodeURI(cleanAddr(targetAddr, false) + ", " +
            targetCity.value.toLowerCase()),
        "destinationURI": sclsLibs.getURI(selected)
      }).then(distanceData => {
        const distanceOrder = sclsLibs.getOrder(selected);
        let distArray = [];
        ...
        branchList.value = distArray[0][0];
        showGMapResponse("Closest Library: " + distArray[0][0], pstatMsg.success);
      }, reject => {
        showGMapResponse(reject.message, pstatMsg.error);
      });
    };
    ...
    if (addrElt && addrEltAlt && cityElt && cityEltAlt) {
      ...
      openTIGERweb.setAttribute('style', 'margin-top:.5em;margin-left:118px;font-size:1.25em;font-weight:bold;font-style:italic;color:' +
          pstatMsg.searching + ';outline:0;text-decoration:none;');
    ...
    const queryPSTAT = function(findAltPSTAT) {
      ...
      chrome.runtime.sendMessage({
        ...
        if (res.hasOwnProperty('error') && res.error) {
          pstatData.errorMsg = res.error;

          countySubCode = getCity(targetCity, false).substring(0,3)
        } else {
          pstatData.matchAddr = res.matchAddr;
          pstatData.county = res.county;
          pstatData.countySub = res.countySub;
          pstatData.censusTract = res.censusTract;
          pstatData.zip = res.zip;

          countySubCode = res.countySub.substring(0,3).toLowerCase();

          pstatData.pstat = (new PSTATs()).find(res.county, res.countySub, res.censusTract);
        }

        if ((!pstatData.pstat || pstatData.pstat === "X-UND")
             && /madison|middleton|verona|monona|fitchburg/i.test(getCity(targetCity, false))) {
          chrome.runtime.sendMessage({
            "key": "getGeneralExceptionsXML",
            "libCode": countySubCode
          }).then(str => {
            if (str.hasOwnProperty('error') && str.error) {
              pstatData.errorMsg = str.error;
            } else {
              let parsedXML = (new window.DOMParser()).parseFromString(str, "text/xml");

              for (let item of parsedXML.getElementsByTagName('address')) {
                let regex = new RegExp(item.getElementsByTagName('regex')[0].textContent, "i");
                if (regex.test(targetAddr.value)) {
                  pstatData.pstat = item.getElementsByTagName('value')[0].textContent;
                  pstatData.zip = item.getElementsByTagName('zip')[0].textContent;
                }
              }

              if ((!pstatData.pstat || pstatData.pstat === "X-UND")
                   && /^mid|ver|sun$/i.test(countySubCode)) {
                chrome.runtime.sendMessage({
                  "key": "getAlderExceptionXML",
                  "libCode": countySubCode
                }).then(str => {
                  if (str.hasOwnProperty('error') && str.error) {
                    pstatData.errorMsg = str.error;
        ...
        function processPstatData(pstatData) {
          ...
          if (pstatData.pstat && pstatData.pstat !== "X-UND" && pstatData.pstat !== "D-MAD-T") {
            if (pstatData.pstat === "MI-NOLIB" || pstatData.pstat === "MI-LIB") {
              ...
              selectList[0].value = pstatData.pstat;
              targetZip.value = pstatData.zip;
              pstatMsg.send(pstatMsg.success, "PSTAT matched with: " + pstatData.matchAddr, findAltPSTAT);
