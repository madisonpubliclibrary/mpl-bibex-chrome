(function(){
  document.getElementById('format').addEventListener('click', e => {
    ...
    print.innerHTML = text.value.replace(/(?:\d{3}-\d{3}-\d{4})|Dream Bus/g, (i => m => !i++ ? "<div>" + m : "<div style=\"page-break-before: always\">" + m)(0)) // All notices
                                ...
                                .replace(/Title:/g, "\n$&") // Hold notices
                                .replace(/If\s+you\s+received\s+this\s+notice\s+via\s+email,?\s+it\s+was\s+sent\s+from\s+a\s+notification-only\s+address\s+that\s+cannot\s+accept\s+incoming\s+mail\.\s+Please\s+Do\s+Not\s+Reply\s+to\s+this\s+message\s+but\s+contact\s+your\s+local\s+library\s+directly\./g,'') // All notices
